...
export default {
  ...
  generateField(field) {
    ...
    if (field.argumentSet.length > 0) {
      acc =
        acc +
        `${this.argumentSetOpeningToken}${this.generateArgumentSet(
          field.argumentSet
        )}${this.argumentSetClosingToken}`;
    }

    if (field.selectionSet.length > 0) {
      acc =
        acc +
        `${this.openingToken}${this.generateSelectionSet(field.selectionSet)}${
          this.closingToken
        }`;
    }
  ...
  generateArgumentSet(set) {
    return set
      .toArray()
      .map((argument) => this.generateArgument(argument))
  ...
  generateArgument({ name, value }) {
    if (typeOf(value) === 'string') {
      ...
      value =
        this.argumentArrayOpeningToken +
        this.wrapArrayInStringTokens(value) +
        this.argumentArrayClosingToken;
    } else if (value instanceof SelectionSet || value instanceof ArgumentSet) {
      value =
        this.argumentObjectOpeningToken +
        this.generateArgumentSet(value) +
        this.argumentObjectClosingToken;
    ...
    return array
      .map((ele) => {
        if (typeof ele === 'object') {
          ...
          eleKeys.forEach((key) => {
            let generatedValue = this.generateArgument({
              name: key,
              value: ele[key],
            });
  ...
  wrapInStringTokens(value) {
    return (
      this.argumentStringWrapperToken + value + this.argumentStringWrapperToken
    );
  },
};
