...
module('unit:ember-graphql-adapter/compiler', function () {
  ...
  test('takes an Model and responds with GraphQL query', function (assert) {
    ...
    assert.strictEqual(
      Compiler.compile(model, store, options),
      'query projectsQuery { projects(status: "active", limit: 10) { id status } }'
    );
  });
  ...
  test('mutation', function (assert) {
    ...
    assert.strictEqual(
      Compiler.compile(model, store, options),
      'mutation projectCreate { projectCreate(name: "Test Project", status: "active") { id name status } }'
    );
  });
  ...
  test('mutation with root alias', function (assert) {
    ...
    assert.strictEqual(
      Compiler.compile(model, store, options),
      'mutation projectCreate { project: projectCreate(name: "Test Project", status: "active") { id name status } }'
    );
  });
  ...
  test('disable automatic selection parsing', function (assert) {
    ...
    assert.strictEqual(
      Compiler.compile(model, store, options),
      'mutation projectDelete { project: projectDelete(id: "667") { id } }'
    );
  });
});
